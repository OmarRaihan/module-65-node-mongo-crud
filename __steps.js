 ...
 * 3. om Server: Create user POST method to receive data on the backend
 * 4. on client side: set fetch with POST, headers, body
 * 5. Make sure you return a json from POST API
 * ------------------------------------
 * Load data to the client server
 * ------------------------------------
 * 1. create get API on the server
 * 2. crete a query object
 * 3. collection.find(query)
 * 4. cursor.toArray()
 * 5. return the result
 * 6. from client useEffect and display like I've done before
 * ---------------------------
 * DELETE
 * --------------------------
 * 
